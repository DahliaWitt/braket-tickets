...
- If logs mention `payment_captured missing balance_transaction`, `refund missing balance_transaction`, or `charge.refunded missing refund object`, keep the Stripe delivery retrying. The handler is waiting for Stripe to expose the BalanceTransaction so the payout ledger can record the real connected-account net.
