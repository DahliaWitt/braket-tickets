...
**Automatic deployment**: The GitHub Actions workflow (`.github/workflows/deploy.yml`) automatically deploys when pushing to `main`. The `deploy-frontend` job builds the Angular app and deploys `dist/frontend/browser` to Cloudflare Pages.
...
Check GitHub Actions for the "Deploy Preview" workflow. Once complete, access `https://dev.community.braket.gay`.
