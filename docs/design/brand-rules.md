...
- **Semantic tokens only.** Use `bg-background`, `text-foreground`, `text-muted-foreground`, and the `primary` / `secondary` / `accent` / `destructive` / `success` / `warning` / `info` families with their `-foreground` / `-text` variants. _Rationale: theming + dark mode + AA calibration all live in the tokens._
...
- **On self-tinted surfaces** (e.g. `bg-warning/10`, `bg-destructive/10`) use the AA-calibrated **`-text` variant** for the foreground (`text-warning-text`, `text-destructive-text`, `text-secondary-text`, `text-info-text`), not the base token. _Rationale: base tokens fail AA over their own tint; the `-text` variants are calibrated for it._
