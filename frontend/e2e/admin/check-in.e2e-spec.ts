...
import {BraAlertDialogHarness} from '../../src/app/ui/components/composites/alert-dialog/alert-dialog.component.harness';
...
test.describe.serial('Check-In Flow', () => {
  ...
  test.describe('Event Selection', () => {
    test(
      ...
      async ({adminPage, convexHelper}) => {
        ...
        const checkInOrgId = await convexHelper.mutation(
        ...
        await expect(
          adminPage.getByRole('heading', {name: 'Check-In'}),
        ).toBeVisible();

        // Get harness (implicitly waits for app-check-in host element to be in DOM)
        const eventSelectionHarness = await createEnvironment(
          adminPage,
        ).getHarness(CheckInComponentHarness);
        ...
        await expect(
          adminPage.getByRole('tab', {name: /Tickets/i}),
        ).toBeVisible();
        await expect(
  ...
  test.describe('Status Display', () => {
    test('check-in shows refunded ticket status', async ({
      adminPage,
      convexHelper,
    }) => {
      const eventTitle = uniqueName('Refunded Ticket Event');
      const refundedTicketOrgId = await convexHelper.mutation(
        api.testing.communities.seedOrganizer,
        {
      ...
      const userId = await convexHelper.mutation(
      ...
      const refundedHarness = await createEnvironment(adminPage).getHarness(
      ...
      await expect(
        ticketRow.getByRole('button', {name: /Check-in/i}),
      ).not.toBeVisible();
      // Should show refunded text somewhere in the row (could be in badge, status text, etc.)
      await expect(
        ticketRow.getByText('refunded', {exact: true}),
      ).toBeVisible();
    });

    test('guest types are displayed correctly', async ({
      adminPage,
      convexHelper,
    }) => {
      const eventTitle = uniqueName('Guest Types Event');
      const guestTypesOrgId = await convexHelper.mutation(
        api.testing.communities.seedOrganizer,
        {
      ...
      const guestTypesHarness = await createEnvironment(adminPage).getHarness(
        CheckInComponentHarness,
      );
      await expect(
        adminPage.locator('select[aria-label="Select event"] option', {
      ...
      await expect(
        adminPage.locator('span').filter({hasText: /^guest$/}),
      ).toBeVisible({
        timeout: 15000,
      });
      await expect(
        adminPage.locator('span').filter({hasText: /^artist guest$/}),
      ).toBeVisible({
        timeout: 15000,
      });
      await expect(
        adminPage.locator('span').filter({hasText: /^staff$/}),
      ).toBeVisible({
        timeout: 15000,
      });
    });

    test('different ticket tiers are displayed correctly', async ({
      adminPage,
      convexHelper,
    }) => {
      const eventTitle = uniqueName('Tier Display Event');
      const tierDisplayOrgId = await convexHelper.mutation(
        api.testing.communities.seedOrganizer,
        {
      ...
      const regularUser = await convexHelper.mutation(
        api.testing.users.createUserDirectly,
        {
      ...
      const supporterUser = await convexHelper.mutation(
        api.testing.users.createUserDirectly,
        {
      ...
      const notaflofUser = await convexHelper.mutation(
        api.testing.users.createUserDirectly,
        {
      ...
      const tierDisplayHarness = await createEnvironment(adminPage).getHarness(
        CheckInComponentHarness,
      );
      await expect(
        adminPage.locator('select[aria-label="Select event"] option', {
      ...
      await expect(
        adminPage.locator('span').filter({hasText: /^regular$/}),
      ).toBeVisible({
        timeout: 15000,
      });
      await expect(
        adminPage.locator('span').filter({hasText: /^supporter$/}),
      ).toBeVisible();
      await expect(
        adminPage.locator('span').filter({hasText: /^notaflof$/}),
      ).toBeVisible();
    });
  });

  test.describe('Check-in actions', () => {
    test('admin can confirm and undo a native ticket check-in', async ({
      adminPage,
      convexHelper,
    }) => {
      const eventTitle = uniqueName('Undo Check-in Event');
      const holderName = uniqueName('Undo Check-in Holder');
      const holderEmail = `${uniqueName('undo-holder')
        .replace(/\s+/g, '-')
        .toLowerCase()}@test.com`;
      const organizerId = await convexHelper.mutation(
        api.testing.communities.seedOrganizer,
        {name: uniqueName('Undo Check-in Org')},
      );
      const eventId = await convexHelper.mutation(
        api.testing.events.seedEvent,
        {
      ...
      await adminPage.goto('/scanner');
      const checkIn = await getHarnessWhenVisible(
        adminPage,
        CheckInComponentHarness,
        15000,
        'attached',
      );
      await checkIn.selectEventByLabel(eventTitle);

      await expect
        .poll(() => checkIn.getCheckInButtonLabels(), {timeout: 15000})
        .toHaveLength(1);
      await checkIn.clickCheckInOnItem(0);
      await expect
        .poll(
          async () =>
            (await checkIn.getManualFeedbackText())?.toLowerCase() ?? null,
          {timeout: 15000},
        )
        .toContain('ticket checked in');
      await expect
        .poll(() => checkIn.getTicketRevertButtonLabels(), {timeout: 15000})
        .toHaveLength(1);

      await checkIn.clickTicketRevertOnItem(0);
      const confirmation = await getHarnessWhenVisible(
        adminPage,
        BraAlertDialogHarness,
        15000,
        'attached',
      );
      expect((await confirmation.getTitleText())?.toLowerCase()).toContain(
        'undo check-in?',
      );
      expect((await confirmation.getOkText())?.toLowerCase()).toBe(
        'undo check-in',
      );
  ...
  test.describe('Filtering', () => {
    test('can filter tickets by name or email', async ({
      adminPage,
      convexHelper,
    }) => {
      const eventTitle = uniqueName('Filter Event');
      const filterOrgId = await convexHelper.mutation(
        api.testing.communities.seedOrganizer,
        {
      ...
      const user1Id = await convexHelper.mutation(
        api.testing.users.createUserDirectly,
        {
      ...
      const user2Id = await convexHelper.mutation(
        api.testing.users.createUserDirectly,
        {
      ...
      const filterTicketsHarness = await createEnvironment(
        adminPage,
      ).getHarness(CheckInComponentHarness);
      await expect(
        adminPage.locator('select[aria-label="Select event"] option', {
    ...
    test('can filter guests by name', async ({adminPage, convexHelper}) => {
      ...
      const guestFilterOrgId = await convexHelper.mutation(
        api.testing.communities.seedOrganizer,
        {
      ...
      const filterGuestsHarness = await createEnvironment(adminPage).getHarness(
        CheckInComponentHarness,
      );
      await expect(
        adminPage.locator('select[aria-label="Select event"] option', {
    ...
    test('shows empty state when no results match filter', async ({
      ...
      const emptyFilterOrgId = await convexHelper.mutation(
        api.testing.communities.seedOrganizer,
        {
      ...
      const emptyFilterHarness = await createEnvironment(adminPage).getHarness(
        CheckInComponentHarness,
      );
      await expect(
        adminPage.locator('select[aria-label="Select event"] option', {
      ...
      await expect(adminPage.getByText('Someone Here')).toBeVisible({
        timeout: 10000,
      });
      ...
      await expect(adminPage.getByText(/No tickets found/i)).toBeVisible({
        timeout: 5000,
      });
    });
  });
