...
function queryAll<T extends Element>(el: { nativeElement: unknown }, selector: string): NodeListOf<T> {
  return (el.nativeElement as HTMLElement).querySelectorAll<T>(selector);
}
function query<T extends Element>(el: { nativeElement: unknown }, selector: string): T | null {
...
describe('ZardTabGroupComponent', () => {
  ...
  it('should set aria-controls pointing to the matching panel id', async () => {
    ...
    for (let i = 0; i < buttons.length; i++) {
      expect(buttons[i].getAttribute('aria-controls')).toBe(panels[i].getAttribute('id'));
    }
  });
  ...
  it('should keep the same tab active when a dynamic list is reordered', async () => {
    ...
    const initialButtons = queryAll<HTMLButtonElement>(fixture, 'button[role="tab"]');
    ...
    const buttons = queryAll<HTMLButtonElement>(fixture, 'button[role="tab"]');
    const activeButton = Array.from(buttons).find((button) => button.getAttribute('aria-selected') === 'true');
  ...
  it('should render pill-style tablist with correct class', async () => {
    ...
    const tabList = query<HTMLElement>(fixture, '[role="tablist"]') as HTMLElement;
