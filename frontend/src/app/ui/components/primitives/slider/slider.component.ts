...
export class ZardSliderComponent extends ControlValueAccessorBase<number> {
  ...
  readonly lastEmittedValue = signal(NaN);
  ...
  constructor() {
    super();
    effect(() => {
      this.disabled.set(this.zDisabled());
    });
    ...
    // Set up pointer event listeners after view is rendered
    afterNextRender(() => {
  ...
  private updateSliderFromPercentage(percentage: number): void {
    ...
    const value = roundToStepClamped(
      rawValue,
      this.zMin(),
      this.zMax(),
      this.zStep(),
