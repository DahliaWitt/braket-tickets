...
@Component({
  template: `<bra-status-badge [status]="status()" [live]="live()">{{
    text()
  }}</bra-status-badge>`,
...
describe('BraStatusBadgeComponent', () => {
  ...
  it('should render accent status', async () => {
    const {harness} = await setup({status: 'accent', text: 'Accent'});
    expect(await harness.getStatus()).toBe('accent');
  });
