...
@Component({
  ...
  imports: [BraCalendarNavigationComponent, BraCalendarGridComponent],
  template: `
    <div [class]="classes()">
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => BraCalendarComponent),
      multi: true,
    },
  ],
...
export class BraCalendarComponent
  extends ControlValueAccessorBase<CalendarValue>
  implements ControlValueAccessor
{
  private readonly destroyRef = inject(DestroyRef);
  private readonly injector = inject(Injector);
  private readonly gridRef = viewChild.required(BraCalendarGridComponent);
  ...
  protected override onChange: (value: CalendarValue) => void = () => undefined;
  ...
  protected onMonthChange(monthIndex: string | string[]): void {
    ...
    if (!monthIndex || monthIndex.trim() === '') {
      logger.warn('Invalid month index received:', monthIndex);
      return;
    }
  ...
  protected onYearChange(year: string | string[]): void {
    ...
    if (!year || year.trim() === '') {
      logger.warn('Invalid year received:', year);
      return;
    }

    const parsedYear = Number.parseInt(year, 10);
    if (Number.isNaN(parsedYear) || parsedYear < 1900 || parsedYear > 2100) {
      logger.warn('Invalid year value:', year, 'parsed as:', parsedYear);
      return;
    }
  ...
  private scheduleGridFocusAfterRender(callback: () => void): void {
