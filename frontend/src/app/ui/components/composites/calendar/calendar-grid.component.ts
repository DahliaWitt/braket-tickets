...
export class BraCalendarGridComponent {
  ...
  readonly previousYear = output();
  readonly nextYear = output();
  ...
  protected getFocusedDayIndex(): number {
    ...
    // Performance optimization: Single pass through days array instead of multiple findIndex calls
    // This reduces from O(3n) to O(n) by checking all conditions in one loop
    const days = this.calendarDays();
    let selectedIndex = -1;
    let todayIndex = -1;
    let firstCurrentMonthIndex = -1;

    for (let i = 0; i < days.length; i++) {
      const day = days[i];
      if (selectedIndex === -1 && day.isSelected) {
        selectedIndex = i;
        // Early return if we found selected date (highest priority)
        return selectedIndex;
      }
      if (todayIndex === -1 && day.isToday && day.isCurrentMonth) {
        todayIndex = i;
      }
      if (firstCurrentMonthIndex === -1 && day.isCurrentMonth && !day.isDisabled) {
        firstCurrentMonthIndex = i;
      }
    }

    // Return in priority order: selected > today > first enabled > 0
  ...
  private setFocus(index: number): void {
    ...
    this.focusedDayIndex.set(index);
    const dayElement = this.gridContainer()?.nativeElement.querySelector<HTMLElement>(
      `#${getDayId(index)}`,
    );
    dayElement?.focus();
  }
