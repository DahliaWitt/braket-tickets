import { SocialAuthBlockedError, type AuthService, type SocialAuthCompletionState } from '@/core/services/auth.service';
...
export async function resolveConfirmOAuthCallback(options: {
  ...
  try {
    const completionState = await options.auth.handleOAuthCallback(options.ott, {
      navigateOnSuccess: false,
      syncUserToApp: options.syncUserToApp,
    });
    ...
    return {
      ok: false,
      errorMessage:
        err instanceof SocialAuthBlockedError ? err.message : options.fallbackErrorMessage,
    };
  }
}
