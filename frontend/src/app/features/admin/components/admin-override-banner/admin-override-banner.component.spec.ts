...
async function setup(options: {
  ...
  harness: AdminOverrideBannerHarness;
}> {
  ...
  fixture.componentInstance.communityName.set(
    options.communityName ?? 'Test Community',
  );
...
describe('AdminOverrideBannerComponent', () => {
  ...
  it('getCommunityNameText() returns the community name', async () => {
    const {harness} = await setup({
      isOverride: true,
      communityName: 'Acid House Collective',
    });
    expect((await harness.getCommunityNameText()).trim()).toBe(
      'Acid House Collective',
    );
  });
  ...
  it('styles the banner with semantic warning tokens (no hardcoded colors)', async () => {
    const {harness} = await setup({isOverride: true});
    const bannerClass = await harness.getBannerClass();
    expect(bannerClass).toContain('bg-warning/10');
    expect(bannerClass).toContain('border-warning/40');
    expect(bannerClass).not.toContain('var(--override');
  });
