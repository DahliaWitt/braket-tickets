...
describe('EventPosterUploaderComponent', () => {
  ...
  describe('currentPosterUrl input', () => {
    it('shows current poster image when a URL is provided', async () => {
      const { harness } = await setup({ currentPosterUrl: 'https://example.com/poster.jpg' });
  ...
  describe('effectivePosterUrl computed', () => {
    it('returns currentPosterUrl when no preview is set', async () => {
      const { component } = await setup({ currentPosterUrl: 'https://example.com/poster.jpg' });
      expect(component.effectivePosterUrl()).toBe('https://example.com/poster.jpg');
    });
  ...
  describe('file selection', () => {
    ...
    it('sets fileName after selecting a file', async () => {
      ...
      const file = new File(['data'], 'awesome-flyer.jpg', { type: 'image/jpeg' });
      const event = { target: { files: [file] } } as unknown as globalThis.Event;
      component.onFileChange(event);
    ...
    it('rejects unsupported files like CSV and shows the accepted formats', async () => {
      ...
      const file = new File(['name,email'], 'attendees.csv', { type: 'text/csv' });
      const event = { target: { files: [file] } } as unknown as globalThis.Event;
      component.onFileChange(event);
    ...
    it('clears an existing local preview when a replacement file is unsupported', async () => {
      const { host, component } = await setup({ currentPosterUrl: 'https://example.com/poster.jpg' });
      const toastSpy = vi.spyOn(toast, 'error').mockImplementation(() => 'toast-id');
      ...
      component.onFileChange({ target: { files: [validFile] } } as unknown as globalThis.Event);

      const invalidFile = new File(['name,email'], 'attendees.csv', { type: 'text/csv' });
      component.onFileChange({ target: { files: [invalidFile] } } as unknown as globalThis.Event);
  ...
  describe('clearFile', () => {
    it('emits null via fileChanged when clearing the selection', async () => {
      ...
      const file = new File(['data'], 'flyer.jpg', { type: 'image/jpeg' });
      component.onFileChange({ target: { files: [file] } } as unknown as globalThis.Event);
  ...
  describe('drag-and-drop', () => {
    ...
    function makeDragEnterEvent(): DragEvent {
      return { preventDefault: vi.fn(), type: 'dragenter' } as unknown as DragEvent;
    }
