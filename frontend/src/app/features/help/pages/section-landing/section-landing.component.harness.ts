...
export class SectionLandingComponentHarness extends ComponentHarness {
  ...
  private getArticleCards = this.locatorForAll('[data-testid="help-article-card"]');
  private getSectionHeadings = this.locatorForAll('h2');
  private getDeveloperVideo = this.locatorForOptional('iframe');
  private getEmptyMessage = this.locatorForOptional('p.text-muted-foreground');
  ...
  async getEmptyStateText(): Promise<string> {
    const msg = await this.getEmptyMessage();
    if (!msg) return '';
    return (await msg.text()).trim();
  }
}
