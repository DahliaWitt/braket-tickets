...
export function magicLinksByCreatorQuery(db: QueryDb, creatorId: Id<'users'>) {
  return db
    .query('magic_links')
    .withIndex('by_createdBy', (q) => q.eq('createdBy', creatorId));
}

export async function loadAllMagicLinksByCreator(
  db: QueryDb,
  creatorId: Id<'users'>,
): Promise<Array<Doc<'magic_links'>>> {
  return await collectAllQueryUnsafe(magicLinksByCreatorQuery(db, creatorId));
}
