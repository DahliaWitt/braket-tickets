...
import {digestBearerToken, generateBearerToken, tokenPrefix} from '../lib/token_digests';
...
export const sendInviteAttempt = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const prepared = await ctx.runMutation(internal.guest_list.invite_state.prepareAttempt, {
    ...
    const attempt = await ctx.runQuery(internal.guest_list.invite_state.loadAttempt, args);
    ...
    try {
      ...
      await sendEmailDeliveryNow(ctx, {
        to: attempt.email,
        subject: message.subject,
        html: message.html,
        requireDelivery: true,
      }, {source: 'guest_list_invite', sourceId: args.assignmentId, recipient: attempt.email, critical: true});
      await ctx.runMutation(internal.guest_list.invite_state.promoteAttempt, args);
    } catch (error) {
