...
/**
 * One-time backfill: register the platform domain on every existing connected
 * account so Apple Pay / Google Pay appear in Embedded Checkout for direct
 * charges. Safe to re-run — `ensurePaymentMethodDomain` is idempotent.
 *
 * Run from the Convex dashboard or CLI, then delete this function.
 */
export async function backfillPaymentMethodDomainsImpl(
  _ctx: ActionCtx,
): Promise<{
  processed: number;
  succeeded: number;
  ...
  if (hostname === 'localhost' || hostname.startsWith('127.')) {
    logger.info(
      'stripe',
      'Skipping payment method domain backfill on localhost',
    );
    return {processed: 0, succeeded: 0, failed: []};
  }

  // V2 accounts appear in the v1 accounts.list endpoint.
  const stripe = getStripeClient();
  ...
  for (const accountId of accounts) {
    try {
      const {applePayStatus, googlePayStatus} = await ensurePaymentMethodDomain(
        accountId,
        hostname,
      );
      results.succeeded++;
      if (applePayStatus !== 'active' || googlePayStatus !== 'active') {
        logger.warn(
          'stripe',
          `Domain registered but not fully active for ${accountId}: apple_pay=${applePayStatus}, google_pay=${googlePayStatus}`,
        );
      }
    } catch (error) {
      ...
      results.failed.push({
        accountId,
        error: String(summary.message ?? JSON.stringify(summary)),
      });
    }
  }

  logger.info('stripe', 'Payment method domain backfill complete', {
    domain: hostname,
    ...results,
